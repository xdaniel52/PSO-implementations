...
class Particle:
    ...
    def Init2(self,particle):
        self.position = particle.position.copy()
        self.value = particle.value
...
class PSO:
    def __init__(self,upper_params,middle_params,lower_params,num_var,epochs,convergence_generation_params = [] ):
    ...
    def Init_particles_from_file(self):
        self.pop_size = self.upper.pop_size+self.middle.pop_size+self.lower.pop_size
        ...
        self.partciels_from_file_counter=0
        self.Init_group_from_file(self.upper.particles,self.upper.pop_size)
        self.Init_group_from_file(self.middle.particles,self.middle.pop_size)
        self.Init_group_from_file(self.lower.particles,self.lower.pop_size)          
    ...
    def Init_group_from_file(self,particles,size):
        particles[:] = []
        for i in range(size):
            position = self.particles_positions[self.partciels_from_file_counter]
            self.partciels_from_file_counter+=1
            velocity = []
    ...
    def Start(self, function, range_of_params, plot_gbest = False, plot_velocity = False,plot_best_worst = False):
        ...
        for e in range(self.epochs):
            ...
            """print()
            print("main")
            print(self.upper.particles[0].value)
            print(self.upper.particles[0].position)
            print(self.upper.particles[0].velocity)
            print()"""
    ...
    def Chose_particles_from_cPop(self):
        self.cPop.sort(key=lambda p: p.value)
        #print(f"best : {self.cPop[0].value}")
        #print(f"worst : {self.cPop[-1].value}")
        #self.worst_history.append(self.cPop[-1].value)
        for i in range(self.upper.pop_size):        
            candidate_id = 1
    ...
    def Update_particles_position_surrogate(self):
    ...
    def Update_particles_position_in_group_surrogate(self, group):
    ...
    def Generate_new_particles(self,epoch):
        self.cpop_size = len(self.cPop)
        self.helper_value=((range_of_params[0][1]-range_of_params[0][0])/20)*((1.0-epoch/self.epochs)**2)
    ...
    def Change_new_particle(self,particle,epoch):
        for dim in range(self.num_var):
    ...
    def Calculate_value_new_particle_surrogate(self,particle):
    ...
    def Update_particles_bests_value(self):
        for i in range(self.upper.pop_size):
    ...
    def Update_particle_bests_value(self,particle):
    ...
    def Plots_before_start(self, plot_gbest,plot_velocity,plot_best_worst):
        if plot_gbest:
    ...
    def Plots_update_history(self, plot_gbest,plot_velocity,plot_best_worst):
        if plot_gbest:
    ...
    def Plots_plot(self, plot_gbest,plot_velocity,plot_best_worst): 
        if plot_gbest:
    ...
    def Convergence_generation_after_epochs(self,plot_gbest,plot_velocity,plot_best_worst):
        if self.convergence_generation:
